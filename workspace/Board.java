...
public class Board extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private void initializePieces() {
        ...
        board[0][7].put(new Pawn(false, RESOURCES_BROOK_PNG));
    ...
    public boolean isInCheck(boolean kingColor) {
        ...
        for (int p = 0; p < oppPieces.size(); p++) {
            if (oppPieces.get(p).getOccupyingPiece().getControlledSquares(board, oppPieces.get(p)).contains(kingSquare)) {
                return true;
            }
        }
        return false;
